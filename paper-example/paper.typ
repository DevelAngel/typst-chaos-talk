...
#let primary = rgb("#21a59d")
#let title = [
  Chaos überwinden ---\
  warum das Werkzeug bestimmt,\
  wie frei Sie denken können
]
...
// #import "mytemplate.typ": mytemplate, maketitle, abstract
// #show: mytemplate.with(
//   head-left: [_On the Typographical Superiority of Typst_],
//   head-right: [Drossos, 2026],
// )
...
#abstract[
  ...
  Drei Sätze, klar im Kopf.
  Aber statt zu schreiben, formatieren Sie --
  Abstände, Schriftgrößen, Seitenränder.
  Der Inhalt rückt in den Hintergrund.
  Das ist kein persönliches Versagen, es ist ein Architekturproblem.
  Dieser Artikel zeigt, wie Typst diese Last vom Autor nimmt:
  Sie liefern den Inhalt, ein Template liefert die Form.
  Und er argumentiert, dass diese Trennung kein Komfortgewinn ist --
  sondern eine Frage der digitalen Souveränität.
]


= Ein Dienstagabend an der Volkshochschule
...
= Mental Load: Das unsichtbare Gewicht der Formatierung
...
#figure(
    ...
    [
      *Textverarbeitungsprogramm*
      ...
      #stack(
        dir: ttb,
        spacing: 4pt,
        rect(width: 100%, fill: primary.lighten(60%), stroke: primary)[Inhalt],
        rect(width: 100%, fill: primary.lighten(60%), stroke: primary)[Schriftart / -größe],
        rect(width: 100%, fill: primary.lighten(60%), stroke: primary)[Abstände / Positionen],
        rect(width: 100%, fill: primary.lighten(60%), stroke: primary)[Seitenränder],
        rect(width: 100%, fill: primary.lighten(60%), stroke: primary)[Konsistenzprüfung],
      )
    ],
    [
      *Satzbasiertes System*

      #stack(
        dir: ttb,
        spacing: 4pt,
        rect(width: 100%, fill: primary.lighten(60%), stroke: primary)[Inhalt],
        rect(width: 100%, fill: gray.lighten(60%), stroke: gray)[
          _übernimmt das Template:_ \
          Schriftart / -größe \
          Abstände / Positionen \
          Seitenränder \
          Konsistenzprüfung
  ...
  caption: [
    Kognitive Entscheidungsebenen beim Erstellen eines Briefs.\
    Farbig: manuelle Entscheidung (Mental Load beim Autor).\
    Grau: automatisiert (Mental Load beim Template).
  ],
  placement: auto,
) <fig:mental-load>
...
= Zwei Philosophien – ein Vergleich
...
#figure(
  {
    ...
    table(
      ...
      inset: 8pt,
      fill: (_, y) => if y == 0 { eastern } else if calc.odd(y) { eastern.lighten(90%) },
      stroke: (x, y) => (
        top: if y <= 1 { 1.5pt + eastern } else { 0.5pt + eastern.lighten(50%) },
        bottom: 0.5pt + eastern.lighten(50%),
        left: none,
        right: none,
      ),
      ...
      [Inhalt-Form-Trennung],     [vermischt],                       [getrennt],
      [Kognitive Ebenen],         [5 gleichzeitig],                  [1 (Inhalt)],
      [Echtzeit-Vorschau],        [ja (ist das Dokument)],           [ja (Live-Vorschau)],
      [Fehlermeldungen],           [keine (stilles Fehlverhalten)],   [lesbar, mit Zeilenverweis],
      [Lernkurve],                [flach, dann Plateau],             [moderat, dann produktiv],
      [Template-Wechsel],         [manuell, fragil],                 [ein Funktionsaufruf],
      [Offenes Format],           [bedingt (ODF)],                   [ja (.typ)],
      [Anbieter-Abhängigkeit],    [hoch],                            [gering],
    )
  },
  caption: [Vergleich zweier Dokumentenwerkzeuge entlang alltagsrelevanter Kriterien.],
  placement: auto,
) <tab:vergleich>
...
= Typst und die Frage der Zugänglichkeit
...
#figure(
  ...
  #show: letter-simple.with(
    sender: (
      name:    "Alex Ausgedacht",
      address: "Zwickauer Straße 41, 09112 Chemnitz",
    ),
    ...
    date: "28. März 2026",
    subject: "Digitale Souveränität in Bildungseinrichtungen",
  )

  Sehr geehrte Damen und Herren,

  ich schreibe Ihnen, weil ich seit vier Jahren als ehrenamtlicher
  Dozent an der Volkshochschule Chemnitz
  Kurse zur digitalen Grundbildung gebe.

  Mit freundlichen Grüßen
  ```,
  caption: [
    Ein vollständiger Brief in Typst.
    Die Konfiguration liest sich wie eine Beschreibung dessen,
    was Sie wollen -- nicht wie eine Anweisung an eine Maschine.
  ],
  placement: bottom,
) <lst:brief>
...
= Was Dokumente mit Souveränität zu tun haben
...
= Drei Schritte, die nichts kosten und alles ändern
...
#slanted-colorbox(
  ...
  und anschließend redaktionell überarbeitet.
]
